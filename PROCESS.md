...
본격 분석 전이지만 받은 응답으로 정성 진단만 가능. **표본**: 50명 중 8명만, 평균 2턴씩 응답.

질문별 응답 현황:

| Q | 의도 | 받은 응답 수 | 평가 |
|---|---|---|---|
| Q1 | 평소 저장 방식 | 5건 | 정상 작동 — 다양한 답변 |
| Q2 | 최근 한 달 방문 사례 | 5건 | 정상 — 전부 "방문 안 함" |
| Q3 | 안 가게 되는 이유 | 1건 | 표본 부족 |
| Q4~Q6 | 서비스 가치 검증 | **0건** | **가장 중요한 부분이 누락** |
| Q7 | 가격 (멀티턴) | 1건 (1턴만) | 표본 부족 |
| Q8 | 약속 불편 사례 | 4건 | **leading question 발견** |

### 잘 작동한 부분

페르소나 캐릭터가 직업·연령·상황에 맞게 잘 변주됨:

- 21세 제빵사 → "퇴근하면 작업실에서 **레시피 노트 정리**"
- 24세 식품가공 조작원 → "퇴근하고 **설비 보전 공부**하느라 피곤"
- 39세 교육 사무원 → "**엑셀 시트에 정리하는 성격**"
- 28세 무직 여성 → "**집 밖으로 나갈 에너지가 많지 않다**"

가식적 "좋아요"가 없고, 솔직한 망설임과 거절이 살아있음. 시스템 프롬프트 설계는 통과.

### 발견된 문제: Q8이 leading question

응답 3건이 거의 동일한 톤으로 "이 앱이 있었으면 편했을 것"이라고 답함. 질문 자체가 정답을 유도하는 구조였음:

> 변경 전: "그때 **이 앱이 있었다면 어떻게 도움이 됐을 것 같나요?**"
> 변경 후: "그때 **어떻게 해결하셨나요?**"

V2에서 우리 앱 언급을 제거하고 기존 해결 방식만 묻도록 수정. 매력 검증은 Q4~Q6에서 따로 처리.

### 가장 의미 있는 한 줄 — 가격 거절 사유

39세 교육 사무원이 결제 거절하며 말한 한 마디:

> "지금도 **스크린샷 찍거나 메모 앱에 적어두는 걸로 충분히 관리**하고 있는데, 굳이 매달 2,900원씩 돈을 내면서까지 써야 할지는 잘 모르겠어요."

**경쟁자 = 다른 장소 저장 앱이 아니라 스크린샷 + 메모앱**이라는 점. 차별화 메시지가 "지도 뷰"만으로는 약할 수 있다. 가치 제안을 더 강하게 다시 짜야 할 수 있음.

이 한 줄 때문에 부분 시뮬도 해 본 보람이 있었음.

### 배운 점

- **n=3에서도 leading question은 잡힌다.** 답변 톤이 너무 일관되게 긍정이면 의심부터.
- **거절 사유는 단순 거부가 아니라 경쟁자 정의의 단서.** "지금도 X로 충분"이라는 답은 우리가 누구와 싸우는지 알려줌.
- **시스템 프롬프트의 1인칭 롤플레잉은 작동한다.** 페르소나 컬럼 3개를 합친 구성이 효과 있었음.
